...
solution =
    let
        { indicesOfSymbols, indicesOfNumbers, numbers } =
            let
                regex =
                    -- Match * or digit sequences
                    Regex.fromString "\\*|\\d+"
                        |> Maybe.withDefault Regex.never
            in
            Regex.find regex (String.trim input)
                |> List.foldl
                    (\curr acc ->
                        case String.toInt curr.match of
                            Just int ->
                                let
                                    span : List Index
                                    span =
                                        List.range curr.index (curr.index + (String.length curr.match - 1))
                                in
                                { acc
                                    | numbers = Dict.insert curr.index int acc.numbers
                                    , indicesOfNumbers =
                                        List.foldl
                                            (\index dict ->
                                                Dict.insert index curr.index dict
                                            )
                                            acc.indicesOfNumbers
                                            span
                                }

                            Nothing ->
                                { acc
                                    | indicesOfSymbols = Set.insert curr.index acc.indicesOfSymbols
                                }
                    )
                    { indicesOfSymbols = Set.empty
                    , indicesOfNumbers = Dict.empty
                    , numbers = Dict.empty
                    }

        collisions : Set ( Id, Id )
        collisions =
            let
                lineLength = 140
            in
            Set.foldl
                (\index acc ->
                    let
                        adjacentSymbolIndices : List Index
                        adjacentSymbolIndices =
                            let
                                north = (index - lineLength - 1 |> Just)
                                northWest = (index - lineLength - 2 |> Just)
                                northEast = (index - lineLength |> Just)
                                east = (Just (index + 1))
                                south = (index + lineLength + 1 |> Just)
                                southEast = (index + lineLength |> Just)
                                southWest = (index + lineLength + 2 |> Just)
                                west = Just (index - 1)
                            in
                            List.filterMap
                                identity
    ...
    Set.foldl
        (\( id1, id2 ) sum ->
            Maybe.map2
                ...
                (Dict.get id1 numbers)
                (Dict.get id2 numbers)
