            ...
            let
                ( firstDigit, lastDigit ) =
                    let
                        regex : Regex
                        regex =
                            Regex.fromString "(\\d|(one|(?<=o)ne|two|(?<=t)wo|three|(?<=t)hree|four|five|six|seven|eight|(?<=e)ight|nine|(?<=n)ine))"
                                |> Maybe.withDefault Regex.never

                        matchToDigit : String -> String
                        matchToDigit str =
                            case str of
                                "one" -> "1"
                                "ne" -> "1" -- Lookbehind
                    ...
                    Regex.find regex line
                        |> List.map .match
                        |> (\matches -> ( Maybe.map matchToDigit (List.head matches), Maybe.map matchToDigit (List.Extra.last matches) ))
            in
            case Maybe.map2 (++) firstDigit lastDigit of
                Just next ->
                    next
                        |> String.toInt
                        |> Maybe.map ((+) acc)
                ...
                Nothing ->
                    acc
    in
    List.foldl accumulate 0 (String.lines input)
