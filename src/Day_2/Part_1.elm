...
solution =
    let
        numbersByColor : String -> String -> List Int
        numbersByColor color line =
            let
                regex : Regex
                regex =
                    Regex.fromString ("\\d+(?=\\s" ++ color ++ ")")
                        |> Maybe.withDefault Regex.never
            in
            Regex.find regex line |> List.filterMap (.match >> String.toInt)

        accumulate : Int -> String -> Int -> Int
        accumulate index line acc =
            let
                gameNumber = index + 1
            in
            Maybe.map3
                ...
                (List.maximum (numbersByColor "red" line))
                (List.maximum (numbersByColor "green" line))
                (List.maximum (numbersByColor "blue" line))
                    |> Maybe.withDefault acc
    in
    input
        |> String.trim
        |> String.lines
        |> List.Extra.indexedFoldl accumulate 0
